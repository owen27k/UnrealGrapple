...
#include "GrappleProjectCharacter.generated.h"
...
UCLASS(config=Game)
class AGrappleProjectCharacter : public ACharacter
{
	GENERATED_BODY()
	...
	// Grapple action input 
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Input, meta = (AllowPrivateAccess = "true"))
	class UInputAction* GrappleAction;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Input, meta = (AllowPrivateAccess = "true"))
	UInputAction* ResetPositionAction;
...
public:
	...
	/* Getter for the bool THESE ARE NOT USED BUT KEPT BECAUSE I DIDN'T WANT TO HAVE TO DEAL WITH DELETING THEM */
	UFUNCTION(BlueprintCallable, Category = Weapon)
	bool GetHasRifle();

	float MinGrappleRange = 100.0f;
	float GrappleRange = 2000.0f;
	float PullSpeed = 2000.0f;
	float MaxSwingSpeed = 2500.0f;
	float InputSwingStrength = 1000.0f;
	bool IsGrappling = false;
	bool IsGrapplePressed = false;
	UMaterialInterface* CableMaterial;
	UMaterialInterface* CableMaterialOOR;
